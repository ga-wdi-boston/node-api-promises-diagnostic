...
const sumLines = (filename, options) => {
...
const sumAllLines = (lines, callback) => {
    let lno = 0;
    let sum = lines.split('\n').reduce((prev, curr, i) => {
      lno = i;
      return prev + (+curr);
...
const sumLinesOfArbitraryLengthFile = (lines, callback) => {
  const rl = require('readline');
  const rli = rl.createInterface({
    input: fs.createReadStream(lines),
  });
  let lno = 0;
  let sum = 0;
  rli.on('line', (line) => {
...
.then(sumAllLines)
.then(sumLinesOfArbitraryLengthFile)
.then(logSum)
.catch(console.error);
