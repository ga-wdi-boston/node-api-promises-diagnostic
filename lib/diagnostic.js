...
sumLines(filename);
  .then(function(lines){
    let lno = 0;
    // I think that there should be a new Promise for what is written below as well, but I'm not quite sure how to go about it.
    let sum = lines.split('\n').reduce((prev, curr, i) => {
        lno = i;
        return prev + (+curr);
      }, 0);
...
const sumLinesOfArbitraryLengthFile = (filename) => {
  // I think there should be a new Promise here as well, but I'm not sure if the const rl and const rli should be inside of the Promise.
