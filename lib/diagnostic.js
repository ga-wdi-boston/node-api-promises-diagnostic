...
// NOTE QUESTION: do we always have to transform a pojo to a json object?

sumLines('data/integers.txt')
.then(JSON.parse)
.then(pojo => pojo)
.then(json => sumLines('data/integers.txt', json))
.catch(console.error)
;
...
sumLinesOfArbitraryLengthFile('data/integers.txt')
.then(JSON.parse)
.then(pojo => pojo)
.then(json => sumLinesOfArbitraryLengthFile('data/integers.txt', json))
.catch(console.error)
;
